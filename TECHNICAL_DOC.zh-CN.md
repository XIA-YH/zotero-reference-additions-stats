...
- 主功能：
  - 通过 `Tools -> Monthly Literature Stats` 打开独立 `文献统计` 标签页
...
- 只向 `Tools` 菜单注入 `Monthly Literature Stats` 菜单项，不再改动左侧集合树
- 点击菜单后通过 `Zotero_Tabs.add({ type: "monthly-stats", title: "文献统计" })` 打开独立统计标签页
- 重复点击菜单会聚焦已有统计页并刷新数据
...
统计页右侧“数据质量”区域显示低优先级调试信息：

- `候选条目`：SQL 返回候选行数
- `有效日期`：成功解析 `dateAdded` 的行数
- `无效日期` / `已排除类型`
- `数据读取错误: ...`：DB 或逻辑异常限制在统计页内展示
...
- [ ] `Tools -> Monthly Literature Stats` 可打开独立 `文献统计` 标签页
- [ ] 左侧集合树不会被替换或隐藏
- [ ] 三种视图都可切换
- [ ] 热力图 `30+` 为最深色
